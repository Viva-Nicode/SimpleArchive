...
final class TableComponentContentView: UIView, UIScrollViewDelegate {
    ...
    private var pageInputActionSubject: PassthroughSubject<MemoPageViewInput, Never>?
    ...
    private func setupUI() {
        ...
        columnAddButton.addAction(
            UIAction { [weak self] _ in
                ...
                pageInputActionSubject?.send(.appendTableComponentColumn(componentID!))
            }, for: .touchUpInside)
        ...
        rowAddButton.addAction(
            UIAction { [weak self] _ in
                guard let self else { return }
                pageInputActionSubject?.send(.appendTableComponentRow(componentID!))
            }, for: .touchUpInside)
    ...
    func configure(
        ...
        self.pageInputActionSubject = pageInputActionSubject
    ...
    func applyColumns(columns: [TableComponentColumn]) {
        ...
        for (i, move) in moves.enumerated() {
            let re = columnsStackView.arrangedSubviews[i] as! TableComponentColumnLabel
            if let move {
                newUIVies[move] = re
                re.text = columns[move].columnTitle
            }
        }
        ...
        adjustTableContentWidthToFit()
        // 만족스럽지 못하지만 O(n)으로 푸는방법 있을 것같은데.. 2차원 DP를 이용해서..
    }
    ...
    @objc private func presentCellEditPopupView(_ gesture: TableCellTapGestureRecognizer) {
        ...
        editCellPopupViewconfirmButtonStore = tableComponentCellEditPopupView
            ...
            .sink { [weak self] newCellValue in
                guard let self else { return }
                pageInputActionSubject?.send(.editTableComponentCellValue(componentID!, id, newCellValue))
            }
    ...
    @objc private func presentColumnEditPopupView(_ gesture: TableColumnTapGestureRecognizer) {
        ...
        pageInputActionSubject?.send(.presentTableComponentColumnEditPopupView(componentID, tappedColumnIndex!))
    }
